...
class EditarEventoPage extends StatefulWidget {
  ...
  EditarEventoPage({required this.postoID, required this.evento});
...
class _EditarEventoPageState extends State<EditarEventoPage> {
  ...
  void loadEventoData() {
    ...
    _descriptionController.text = widget.evento.descricao;
    _moradaController.text = widget.evento.morada;
    _telemovelController.text = widget.evento.telemovel?.toString() ?? '';
    _emailController.text = widget.evento.email ?? '';
    selectedAreaId = widget.evento.idArea;
    selectedSubareaId = widget.evento.idSubarea;
  ...
  Future<void> _updateEvent() async {
    ...
    if (showValidationDate == false && (_selectedDate != null || _selectedTime != null)) {
      setState(() {
    ...
    if (isValidated) {
      ...
      final formattedHora = _selectedTime != null
        ? '${_selectedTime!.hour.toString().padLeft(2, '0')}:${_selectedTime!.minute.toString().padLeft(2, '0')}:00'
        : null;

      final api_eventos = EventosAPI();
      final response = await api_eventos.editarEvento(
        id: widget.evento.id,
        titulo: _titleController.text.isNotEmpty ? _titleController.text : null,
        descricao: _descriptionController.text.isNotEmpty ? _descriptionController.text : null,
        data: _selectedDate,
        hora: formattedHora,
        morada: _moradaController.text.isNotEmpty ? _moradaController.text : null,
        telemovel: _telemovelController.text.isNotEmpty
            ? int.tryParse(_telemovelController.text)
            : null,
            ...
            child: Column(
              children: [
                ...
                TextInput(
                    ...
                    label: 'Descrição',
                    keyboardType: TextInputType.text,
                    errorMessage: 'Por favor, insira a descrição'),
                const SizedBox(height: 15),
                TextInput(
                ...
                TextInput(
                    ...
                    errorMessage: 'Por favor, insira a morada'),
                const SizedBox(height: 15),
                DateTimeInput(
                  onDateChanged: (date) {
                    _selectedDate = date;
                ...
                DropdownInput<int>(
                  ...
                  label: 'Área',
                  validator: (value) => validateNotEmpty(value?.toString(),
                ...
                DropdownInput<int>(
                  ...
                  label: 'Subárea',
                  validator: (value) => validateNotEmpty(value?.toString(),
                      errorMessage: 'Por favor, selecione uma subárea'),
                ),
                const SizedBox(height: 15),
                ImageInput(
                    selectedImage: _selectedImage,
                    onPickImage: _pickImage,
                    validator: isImageNull),
